...
-- которые предоставляют методы stomp (stab) и
...
-- которая вызывает метод stomp, если переданное ему значение
-- приводит в ярость Морка; вызывает stab, если оно приводит
-- в ярость Горка и вызывает сначала stab, а потом stomp,
